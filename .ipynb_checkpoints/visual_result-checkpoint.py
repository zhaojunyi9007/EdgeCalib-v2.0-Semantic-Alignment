            ...
            with open(points_file, 'r') as f:
                for line in f:
                    if line.strip() and not line.startswith('#'):
                        ...
                        if len(vals) >= 7:  # x y z i nx ny nz
        ...
        with open(calib_result_file, 'r') as f:
            ...
            for idx, line in enumerate(lines):
                ...
                if line.startswith('Rotation (rx, ry, rz):'):
                    # 解析格式: "Rotation (rx, ry, rz): 0.123 0.456 0.789"
                    parts = line.split(':')
                    if len(parts) == 2:
                    ...
                    # 解析格式: "Translation (tx, ty, tz): 1.0 2.0 3.0"
                    parts = line.split(':')
                    if len(parts) == 2:
                        vals = list(map(float, parts[1].strip().split()))
                        if len(vals) == 3:
                ...
                elif line.startswith('# Rotation (angle-axis):'):
                    # 解析格式: 
                    # # Rotation (angle-axis): rx ry rz
                    # <rx> <ry> <rz>
                    if idx + 1 < len(lines):
                        vals = list(map(float, lines[idx + 1].strip().split()))
                        if len(vals) == 3:
                            r_vec = np.array(vals)
                elif line.startswith('# Translation (meters):'):
                    # 解析格式:
                    # # Translation (meters): tx ty tz
                    # <tx> <ty> <tz>
                    if idx + 1 < len(lines):
                        vals = list(map(float, lines[idx + 1].strip().split()))
                        if len(vals) == 3:
            ...
            if r_vec is not None and t_vec is not None:
                print(f"[Info] Loaded calibration result from {calib_result_file}")
                return r_vec, t_vec
    except Exception as e:
